...
> 每个对象都是由管理大块内存的Span切割而来的，而一个正在使用中的Span，切割的单元大小是固定的。因此只需在切割后标识Span的切割单元大小（为Span类新增一个成员`objSize`），内存对象`obj`释放时，通过哈希映射找到其所属的Span，就能知道自己的大小是多少。注意：大块内存分配时没有切割，要特殊处理，其整体就是一个内存对象。
